...
# A local safety net for visible evidence.  AI-provided quotes remain the
# primary highlights, but these patterns ensure obvious risks (links, OTPs,
# payment demands, and urgency) are still visible if the model omits a quote.
_IMPORTANT_TEXT_PATTERNS = (
    r"https?://[^\s]+|www\.[^\s]+|\b(?:[a-z0-9-]+\.)+(?:com|in|org|net|info|co)\b",
    r"\b(?:otp|pin|password|cvv|aadhaar|account number|bank details)\b",
    r"\b(?:pay|payment|processing fee|transfer|send money|upi)\b",
    r"\b(?:urgent|immediately|act now|within \d+ (?:hours?|days?)|limited time|expires?)\b",
    r"\b(?:account (?:will be )?(?:blocked|closed)|verify (?:now|immediately)|click here)\b",
)
...
def render_highlighted_message(text: str, all_quotes: list[str]) -> None:
    ...
    local_terms = [
        match.group(0)
        for pattern in _IMPORTANT_TEXT_PATTERNS
        for match in re.finditer(pattern, text, flags=re.IGNORECASE)
    ]
    highlight_terms = list(dict.fromkeys([*all_quotes, *local_terms]))

    if not highlight_terms:
        # No highlights — render in a plain pre-formatted box
        st.markdown(
            f'<div class="scam-card" style="background-color:#f8f9fa; border:1px solid #dee2e6; '
    ...
    sorted_quotes = sorted(set(highlight_terms), key=len, reverse=True)
    ...
    st.markdown(
        f'<div class="scam-card" style="background-color:#f8f9fa; border:2px solid #f39c12; '
        ...
        with st.expander(f"{icon} {tactic.tactic_name}", expanded=False):
            ...
            if tactic.evidence_quotes:
                ...
                for quote in tactic.evidence_quotes:
                    ...
                    st.markdown(
                        f'<blockquote class="scam-card" style="'
