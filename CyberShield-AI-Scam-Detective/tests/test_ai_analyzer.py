...
def _make_mock_client(content: str) -> MagicMock:
    """Return a mock Gemini client that returns `content` from Interactions."""
...
class TestAIAnalyzer:
    ...
    def test_api_exception_returns_fallback(self):
        ...
        mock_client = MagicMock()
        mock_client.interactions.create.side_effect = RuntimeError("connection failed")
    ...
    def test_fallback_debug_redacts_api_key_pattern(self):
        ...
        mock_client.interactions.create.side_effect = RuntimeError(
...
class TestRetry:
    ...
    def test_transient_error_retried_then_succeeds(self):
        ...
        mock_client.interactions.create.side_effect = [
            exc_503,
            MagicMock(output_text=_valid_ai_json(risk_level="High")),
        ]
        analyzer = AIAnalyzer(client=mock_client)
    ...
    def test_transient_error_exhausts_retries_returns_fallback(self):
        """The preferred model exhausts retries and returns a fallback."""
        exc_503 = self._make_503()
        mock_client = MagicMock()
        mock_client.interactions.create.side_effect = [exc_503] * 3
        analyzer = AIAnalyzer(client=mock_client)
        with patch("ai.ai_analyzer.time.sleep"):
            result = analyzer.analyze("Test message.")
        assert result.is_fallback is True
    ...
    def test_404_on_custom_model_uses_current_default(self):
        """A stale custom model automatically falls back to the supported default."""
        ...
        mock_client.interactions.create.side_effect = [
            exc_404,
            MagicMock(output_text=_valid_ai_json(risk_level="Medium")),
        ]
        analyzer = AIAnalyzer(client=mock_client, model="custom-retired-model")
        with patch("ai.ai_analyzer.time.sleep"):
            result = analyzer.analyze("Test message.")

        assert result.is_fallback is False
        assert mock_client.interactions.create.call_args_list[-1].kwargs["model"] == "gemini-3.6-flash"

    def test_retired_model_name_is_migrated(self):
        """Legacy GEMINI_MODEL values do not make retired API requests."""
        analyzer = AIAnalyzer(client=_make_mock_client(_valid_ai_json()), model="models/gemini-2.5-flash")
        result = analyzer.analyze("Test message.")
        assert result.is_fallback is False
        assert analyzer._model_name == "gemini-3.6-flash"
    ...
    def test_permanent_error_not_retried(self):
        ...
        mock_client = MagicMock()
        mock_client.interactions.create.side_effect = exc_404
        analyzer = AIAnalyzer(client=mock_client)
    ...
    def test_auth_error_not_retried(self):
        ...
        mock_client = MagicMock()
        mock_client.interactions.create.side_effect = exc_401
        analyzer = AIAnalyzer(client=mock_client)
        with patch("ai.ai_analyzer.time.sleep") as mock_sleep:
    ...
    def test_sleep_called_between_retries(self):
        ...
        mock_client.interactions.create.side_effect = [
            exc_503,
            exc_503,
            MagicMock(output_text=_valid_ai_json(risk_level="Low")),
        ]
        analyzer = AIAnalyzer(client=mock_client)
        with patch("ai.ai_analyzer.time.sleep") as mock_sleep:
            analyzer.analyze("Test message.")
