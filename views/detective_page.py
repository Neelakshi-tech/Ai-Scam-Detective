...
def render() -> None:
    ...
    st.markdown(
        ...
        <div class="hero-panel">
            <div class="eyebrow">Practice makes safer choices</div>
            <h1 style="margin:6px 0;">🕵️ Detective Mode</h1>
            <p>Train your scam-spotting skills using safe fictional examples. Select every suspicious clue,
            then get an explanation and score.</p>
            <div class="hero-meta">● TRAINING SIMULATION &nbsp;&nbsp; • &nbsp;&nbsp; NO REAL DATA &nbsp;&nbsp; • &nbsp;&nbsp; LEVEL UP YOUR DEFENCE</div>
        </div>
        """, unsafe_allow_html=True,
    )
    st.info("Tip: Look for pressure, requests for money or private details, and links that do not match the claimed organisation.", icon="💡")
    ...
    # Show the message
    # ------------------------------------------------------------------
    st.markdown("### 1. Read this sample message")
    st.markdown(
        ...
        f"{html.escape(scenario.message_text)}</div>",
        unsafe_allow_html=True,
    )
    ...
    # Phrase selection
    # ------------------------------------------------------------------
    st.markdown("### 2. Select the suspicious clues")
    st.caption("There can be more than one. Choose your best answer, then click Submit My Answers.")
...
def _render_result(result, scenario, selected_indices: list[int]) -> None:
    ...
    for i, phrase in enumerate(scenario.phrases):
        ...
        if is_correct and was_selected:
            st.markdown(
                f'<div class="scam-card" style="background:#eafaf1; border-left:4px solid #27ae60; '
            ...
            st.markdown(
                f'<div class="scam-card" style="background:#fdf0ef; border-left:4px solid #e74c3c; '
                f'padding:8px 14px; margin:4px 0; border-radius:4px;">'
                f'❌ <strong>"{phrase}"</strong> — '
            ...
            st.markdown(
                f'<div class="scam-card" style="background:#fef9e7; border-left:4px solid #f39c12; '
                f'padding:8px 14px; margin:4px 0; border-radius:4px;">'
                f'⚠️ "{phrase}" — '
            ...
            st.markdown(
                f'<div class="scam-card" style="background:#f8f9fa; border-left:4px solid #dee2e6; '
                f'padding:8px 14px; margin:4px 0; border-radius:4px;">'
...
# Make Streamlit's automatic /detective_page route render actual content.
if __name__ == "__main__":
    st.set_page_config(page_title="Detective Mode | AI Scam Detective", page_icon="🕵️", layout="centered")
